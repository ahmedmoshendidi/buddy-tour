...
const getTourById = async (req, res) => {
  ...
  try {
    ...
    const timeSlotsRes = await pool.query(
      `SELECT date, time, capacity, booked_seats, available_spots, language FROM time_slots WHERE tour_id = $1 ORDER BY date, time`,
      [id]
    );
...
const getTourBySlug = async (req, res) => {
  ...
  try {
    ...
    const timeSlotsRes = await pool.query(
      `SELECT date, time, capacity, booked_seats, available_spots, language
       FROM time_slots
       WHERE tour_id = $1
       ORDER BY date, time`,
      [tourId]
