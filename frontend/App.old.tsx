...
useEffect(() => {
  ...
  const handleViewTourDetails = (tour: Tour) => {
    setSelectedTourSlug(tour.slug || tour.id.toString()); // fallback لو مفيش slug
    setCurrentView('tour-details');
  };

  // Helper for footer links that use tour IDs
  const handleViewTourById = (tourId: number) => {
    setSelectedTourSlug(tourId.toString());
    setCurrentView('tour-details');
  };
  ...
  const handleBookNow = (tour: Tour) => {
    setSelectedTourSlug(tour.slug || tour.id.toString());
    setCurrentView('tickets');
  };
  ...
  const handleToggleFavorite = (e: React.MouseEvent, tour: Tour) => {
    ...
    const tourForFavorites = {
      ...
      duration: tour.duration || '2 hours', // Ensure duration is always a string
      max_group_size: tour.max_group_size || 10, // Ensure max_group_size is always a number
      rating: tour.rating || 4.8, // Ensure rating is always a number
      reviews_count: tour.reviews_count || 100 // Ensure reviews_count is always a number
    };
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-8">
            ...
            <div>
              ...
              <ul className="space-y-2 text-sm text-muted-foreground">
                <li><button onClick={() => handleViewTourById(1)} className="hover:text-primary transition-colors">Bibliotheca Alexandrina</button></li>
                <li><button onClick={() => handleViewTourById(2)} className="hover:text-primary transition-colors">Roman Theatre</button></li>
                <li><button onClick={() => handleViewTourById(3)} className="hover:text-primary transition-colors">Montaza Palace</button></li>
                <li><button onClick={() => handleViewTourById(4)} className="hover:text-primary transition-colors">Qaitbay Citadel</button></li>
              </ul>
            </div>
